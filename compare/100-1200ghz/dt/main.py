import os
import numpy as np
import matplotlib.pyplot as plt

from sklearn.tree import DecisionTreeRegressor
from sklearn.model_selection import GridSearchCV
from sklearn.metrics import mean_squared_error


# ============================================================
# Config
# ============================================================
class Config:
    MODEL_TYPE = "DecisionTree"
    RESULT_SAVE_DIR = os.path.dirname(os.path.abspath(__file__))


# ============================================================
# 1. 读取 CSV 数据
# ============================================================
def load_csv(path):
    """
    CSV:
        第1列: 频率测量值 (Measured Frequency)
        第2列: 频率真实值 (True Frequency)
        第1行为表头
    """
    data = np.loadtxt(
        path,
        delimiter=",",
        skiprows=1,
        usecols=(0, 1)
    )
    X = data[:, 0:1]
    y = data[:, 1:2]
    return X, y


X_train, y_train = load_csv("./input/20260113/train.csv")
X_eval,  y_eval  = load_csv("./input/20260113/eval.csv")


# ============================================================
# 2. Decision Tree + GridSearch
# ============================================================
base_model = DecisionTreeRegressor(
    random_state=42
)

param_grid = {
    "max_depth": [3, 5, 7, 9, 12, None],
    "min_samples_leaf": [1, 3, 5, 10],
    "min_samples_split": [2, 5, 10],
    "ccp_alpha": [0.0, 1e-5, 1e-4, 1e-3],
}

search = GridSearchCV(
    estimator=base_model,
    param_grid=param_grid,
    cv=5,
    scoring="neg_mean_squared_error",
    n_jobs=-1,
    verbose=1
)

search.fit(X_train, y_train.ravel())

print("Best DT params:", search.best_params_)
print("Best CV MSE:", -search.best_score_)

model = search.best_estimator_


# ============================================================
# 3. 预测 & 残差
# ============================================================
y_pred = model.predict(X_eval).reshape(-1, 1)

meas_residuals = X_eval - y_eval      # 校准前
pred_residuals = y_pred - y_eval      # 校准后

mse = mean_squared_error(y_pred, y_eval)
mae = np.mean(np.abs(pred_residuals)) * 1000  # 转换为 MHz
rmse = np.sqrt(mse) * 1000  # 转换为 MHz

print(f"Eval MAE: {mae:.4f} MHz")
print(f"Eval MSE: {mse:.6f} GHz^2")
print(f"Eval RMSE: {rmse:.4f} MHz")

# ============================================================
# 图 1：3 子图综合分析
# ============================================================
plt.figure(figsize=(8, 12))

# 1️⃣ Feature vs Target
plt.subplot(3, 1, 1)
plt.scatter(X_train, y_train, alpha=0.6, label="Train")
plt.scatter(X_eval, y_eval, alpha=0.6, label="Eval")
plt.title("Feature vs Target")
plt.xlabel("Frequency (GHz)")
plt.ylabel("True Frequency (GHz)")
plt.legend()

# 2️⃣ Actual vs Predicted
plt.subplot(3, 1, 2)
plt.scatter(y_eval, y_pred, alpha=0.6)
min_y, max_y = y_eval.min(), y_eval.max()
plt.plot([min_y, max_y], [min_y, max_y], linestyle="--", color="black")
plt.title("Actual vs Predicted")
plt.xlabel("Actual")
plt.ylabel("Predicted")

# 3️⃣ Residuals
plt.subplot(3, 1, 3)
plt.scatter(X_eval, pred_residuals, alpha=0.6)
plt.axhline(0, linestyle="--", color="black")
plt.title("Residuals (Eval)")
plt.xlabel("Frequency (GHz)")
plt.ylabel("Residuals (GHz)")

plt.tight_layout()
plt.savefig(
    os.path.join(
        Config.RESULT_SAVE_DIR,
        "decision_tree_eval_overview.png"
    )
)
plt.close()


# ============================================================
# 图 2：频率残差对比（MHz）
# ============================================================
x_freq = X_eval.squeeze()
idx = np.argsort(x_freq)

plt.figure(figsize=(8, 5))
plt.plot(
    x_freq[idx],
    meas_residuals.squeeze()[idx] * 1000,
    label="MEASURED ERROR"
)
plt.plot(
    x_freq[idx],
    pred_residuals.squeeze()[idx] * 1000,
    label="PREDICTED ERROR"
)

plt.axhline(0, linestyle="--", color="black")
plt.xlabel("Frequency (GHz)")
plt.ylabel("Residuals (MHz)")
plt.title("Decision Tree Frequency Residuals")
plt.grid(True)

meas_mae = np.mean(np.abs(meas_residuals)) * 1000
pred_mae = np.mean(np.abs(pred_residuals)) * 1000

plt.text(
    0.05, 0.95,
    f"MEAS_MAE: {meas_mae:.4f} MHz\nPRED_MAE: {pred_mae:.4f} MHz",
    transform=plt.gca().transAxes,
    verticalalignment="top",
    bbox=dict(boxstyle="round", facecolor="white", alpha=0.8)
)

plt.legend()
plt.tight_layout()
plt.savefig(
    os.path.join(
        Config.RESULT_SAVE_DIR,
        "residual_plot_DecisionTree.png"
    )
)
plt.close()
